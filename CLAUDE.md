...
- **Read payload cap (`read_too_large`).** The node budget bounds node *count*, not payload *bytes* — a small-but-deep full serialize (e.g. 59 nodes × ~3.7k chars = 219KB) sailed under the 8MB WS cap but overflowed the MCP client's token budget AND re-cost context every turn. `ws-server.ts` rejects a read result over `READ_SOFT_CAP_BYTES` (env `GRIP_READ_CAP_BYTES`, default 120KB) for read methods with an actionable `read_too_large` naming the remedy (pass `properties`, lower `depth`, or `export_node` JSON to disk). Distinct from the 8MB `response_too_large` OOM backstop. A `properties` whitelist recurses to every node in the tree (not just the root) — pass e.g. `properties:['name','variantProperties']` for a lean recursive read; it also skips the per-INSTANCE `getMainComponentAsync` unless `componentId`/`componentName` is requested.
- **Perf gotcha — first touch of a REMOTE (library) component set is slow.** The first `set_node_property componentProperties` / `swap_instance` / variant `setProperties` against a remote component set makes Figma load that set (~1–2s); subsequent touches are ms. This cost is Figma-inherent (loading the remote set) — grip can't shrink a single first-touch. It only bit as a *batch* failure (5 sequential swaps overran the 10s budget on the cold load); that's now covered by run_script's opt-in `timeoutMs` (max 120s). If you still want each call fast: split the first swap into its own call (warms it), or touch one instance of the set first, then batch the rest.
- **A `set_active_file` (or `GRIP_FILE`) binding survives plugin reconnects; auto-route does not.** `mcp.bind` is only ever set by `set_active_file`/`bindFromLaunch` and is never cleared on reconnect — `route()` clears just the resolved `activeFileId` and re-resolves it from the durable `bind` (newest live session for that fileKey). An UNBOUND agent (relying on single-open-file auto-routing) has nothing to survive, so during reconnect churn its calls can hit the retryable `plugin_reconnected` — bind to avoid that window. (A `bind: null` in `get_page_context` means the agent never bound, not that a binding was lost.)
...
150 tools. Per-tool reference lives in [`TOOLS.md`](./TOOLS.md). The default advertised scope is now `core` (~35 tools) rather than all 150; agents widen via `GRIP_TOOLS`/`?tools=` (categories from `grip_capabilities`), and `run_script` reaches any tool regardless of scope. `grip_capabilities` also takes `{tool:'<name>'}` to return that single tool's relocated `detail` (the deep guidance trimmed out of the always-injected `description`). Phase 3 merged 19 clusters of "N verbs on one object" into single op-enum-dispatched tools (e.g. `table_op {op}`, `bind_to_variable {target}`, `timer {op}`) — the former per-verb tools (`table_insert_row`, `group_nodes`, `bind_paint_to_variable`, …) no longer exist as standalone TOOLS entries; call them via the merged tool's `op`/`target` discriminator. Per-op params are in `grip_capabilities {tool:'<name>'}`. Quick map:

**Bridge-side (no plugin round-trip):**
- `list_files`, `set_active_file` — multi-file session routing.

**Read:** `get_document`, `get_page`, `get_page_context` (cheap "where am I": file/page/selectionCount), `list_pages` (cheap page enumeration, no content load), `get_audit` (one-pass document summary — counts by type, fill-source breakdown, fonts, styles/variables), `get_node`, `get_nodes` (bulk), `get_selection`, `get_styles`, `get_variables`, `get_components`, `search_nodes` (now takes `properties` to project fields per match instead of a bare id/name/type/parentId hit), `get_plugin_data`.

**Export:** `export_node` (SVG, PNG, JPG, PDF, CSS, JSON).

**Write — node-level:** `set_node_property` (one dispatcher across ~60 props — numeric/bool/passthrough buckets), `create_node` (10 types incl. GROUP/LINE/POLYGON/STAR/VECTOR; `props` payload applied at creation), `create_tree` (build a nested subtree from one recursive `{type,props?,children?}` spec in a single call — reuses `create_node`'s factory + `set_node_property`; capped 2000 nodes), `delete_node`, `clone_node`, `move_node`, `group` (merged; `op:'group'|'ungroup'`).
...
**Write — text range:** `set_text_range_property` (per-range styling on TEXT nodes), `replace_text` (bulk find/replace across TEXT nodes in a bounded, font-loading loop — no `run_script`; literal unless `regex:true`; scope: page/subtree/pageId/allPages).
...
- **`node_not_found` fast-fail.** `figma.getNodeByIdAsync` HANGS forever on a removed/invalid id (Figma quirk) instead of returning null. The shared `getNode(id)` helper (used by every node-targeting handler) races it against `GETNODE_TIMEOUT_MS` (8s) → throws a typed `node_not_found` naming the likely cause (removed) plus the recovery for the rare false-positive (a valid node on an unloaded heavy page — make its page current first). This is why `get_node`/`get_nodes`/etc. no longer eat the full budget on a stale id.
